...
export default function Navbar({ currentTime }) {
  ...
  const navItems = [
    ...
    { label: 'DÉPLOIEMENT', href: 'déploiement' },
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-perfex-bg/95 backdrop-blur-sm border-b-2 border-black">
      <div className="flex items-stretch">
        ...
        <button
          ...
          className="px-8 py-4 bg-black text-white text-xs font-bold tracking-widest hover:bg-perfex-red transition-colors flex items-center gap-2 group"
        >
        ...
        <div className="lg:hidden border-t-2 border-black">
          {navItems.map((item, i) => (
            <button
              key={i}
              onClick={() => scrollToSection(item.href)}
              className="block w-full text-left px-6 py-4 border-b border-black/10 text-sm font-bold tracking-widest hover:bg-black hover:text-white transition-colors"
            >
              {item.label}
            </button>
          ))}
